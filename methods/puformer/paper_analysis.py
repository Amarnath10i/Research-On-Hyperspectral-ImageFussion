...
from hsifuse.metrics import evaluate, onions_quality, q2n
from hsifuse.ops import CHIKUSEI_WL, PAVIA_WL, Degradation, dataset_srf

RGB = {"chikusei": (60, 40, 20), "pavia": (57, 34, 3)}   # false-colour bands used in TIP'26
...
def main():
    ...
    ap.add_argument("--dataset", default="chikusei", choices=["chikusei", "pavia"])
    ...
    first = [p for p in find("test_pred.npy") if "first_run" in p]
    preds = {
    ...
    if first:
        preds["PUFormer (first run)"] = torch.from_numpy(np.load(first[0])).double()
    preds["PUFormer v3" if a.dataset == "chikusei" else "PUFormer"] = torch.from_numpy(np.load(v3)).double()
    final = list(preds)[-1]
    ...
    for name, x in preds.items():
        ...
        per_band[name] = dict(psnr=(10 * torch.log10(1 / mse)).tolist(), rmse_dn=(mse.sqrt() * DN).tolist(),
    ...
    json.dump(dict(per_band=per_band, overall=overall, wavelengths_nm=WL[a.dataset].tolist(), dataset=a.dataset, dn_scale=DN),
              open(os.path.join(a.out, "per_band.json"), "w"))

    # 2) Q2n analysis of the final model
    p = preds[final]
    res = {"Q2n": float(np.mean([q2n(gt[i], p[i]) for i in range(n_img)]))}
    for name, bands in (("bands_0_7_exact", range(0, 8)), ("bands_0_7_104_127_exact", list(range(0, 8)) + list(range(B - 24, B)))):
        pp = p.clone(); pp[:, list(bands)] = gt[:, list(bands)]
        res[name] = float(np.mean([q2n(gt[i], pp[i]) for i in range(n_img)]))
    qs, stds, errs = zip(*[block_q(gt[i], p[i]) for i in range(n_img)])
    ...
    res["relative_error_worst10pct_by_group_of_8_bands"] = [float(c.mean()) for c in torch.split(rel, 8)]
    res["per_tile_Q2n"] = [float(q2n(gt[i], p[i])) for i in range(n_img)]
    ...
    A = torch.cat([hp_ms, torch.ones_like(hp_ms[:, :1])], 1).permute(0, 2, 3, 1).reshape(-1, ms.shape[1] + 1)
    oracle = {}
    for b in list(range(10)) + [B // 2, B - 8, B - 1]:
        y = hp_gt[:, b].reshape(-1, 1)
        w = torch.linalg.lstsq(A, y).solution
        oracle[b] = dict(model=float((p[:, b] - gt[:, b]).pow(2).mean().sqrt() * DN),
                         oracle_linear=float((y - A @ w).pow(2).mean().sqrt() * DN),
                         bicubic=float(hp_gt[:, b].pow(2).mean().sqrt() * DN))
    res["oracle_injection_rmse_dn"] = oracle
    calib = []
    for alpha in (0.0, 0.02, 0.05, 0.1, 0.2, 0.35):
        x = (p + alpha * (preds["Bicubic"] - p)).clamp(0, 1)
        calib.append(dict(alpha=alpha, PSNR=float(np.mean([10 * np.log10(1 / float((x[i] - gt[i]).pow(2).mean())) for i in range(n_img)])),
                          Q2n=float(np.mean([q2n(gt[i], x[i]) for i in range(n_img)]))))
    ...
    np.savez_compressed(os.path.join(a.out, "visual.npz"), rgb_bands=np.array(rgb), gt=comp(gt),
                        ...
                        **{f"mae_{i}": ((x - gt).abs().mean(1) * DN).float().numpy().astype(np.float16) for i, x in enumerate(preds.values())},
